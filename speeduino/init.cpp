 ...
 * - Load config from EEPROM, update config structures to current version of SW
 * if needed.
 * - Initialise board (The initBoard() is for board X implemented in board_X.ino
 * file)
 * - Initialise timers (See timers.ino)
 * - Perform optional SD card and RTC battery inits
 * - Load calibration tables from EEPROM
 * - Perform pin mapping (calling @ref setPinMapping() based on @ref
 * config2.pinMapping)
 * - Stop any coil charging and close injectors
 * - Initialise schedulers, Idle, Fan, auxPWM, Corrections, AD-conversions,
 * Programmable I/O
 ...
 *   - Assign injector open/close and coil charge begin/end functions to their
 * dedicated global vars
 ...
 * - Mark Initialisation completed (this flag-marking is used in code to prevent
 * after-init changes)
 */
...
*/
  ...
  while (digitalRead(EEPROM_RESET_PIN) != HIGH &&
         (millis() - start_time) < 1050) {
    ...
    if ((millis() - start_time) > 500) {
      ...
      // see if the user reacts to the led turned off with removing the keypress
      // within 1 second
  ...
  loadConfig();
  doUpdates();  // Check if any data items need updating (Occurs with firmware
                // updates)
#endif
  ...
  initBoard();  // This calls the current individual boards init function. See
                // the board_xxx.ino files for these.
  initialiseTimers();
  ...
  Serial.begin(115200);
  BIT_SET(currentStatus.status4,
          BIT_STATUS4_ALLOW_LEGACY_COMMS);  // Flag legacy comms as being
                                            // allowed on startup
  ...
  PrimingPulseTable.axisSize =
      SIZE_BYTE;  // Set this table to use byte axis bins
  ...
  injectorVCorrectionTable.axisSize =
      SIZE_BYTE;  // Set this table to use byte axis bins
  ...
  IATDensityCorrectionTable.axisSize =
      SIZE_BYTE;  // Set this table to use byte axis bins
  ...
  idleAdvanceTable.axisSize = SIZE_BYTE;  // Set this table to use byte axis
                                          // bins
  ...
  rotarySplitTable.axisSize = SIZE_BYTE;  // Set this table to use byte axis
                                          // bins
  ...
  flexBoostTable.axisSize =
      SIZE_BYTE;  // Set this table to use byte axis bins (NOTE THIS IS
  ...
  knockWindowStartTable.axisSize =
      SIZE_BYTE;  // Set this table to use byte axis bins
  ...
  knockWindowDurationTable.axisSize =
      SIZE_BYTE;  // Set this table to use byte axis bins
  ...
  oilPressureProtectTable.axisSize =
      SIZE_BYTE;  // Set this table to use byte axis bins
  ...
  rollingCutTable.axisSize =
      SIZE_SIGNED_BYTE;  // X axis is SIGNED for this table.
  ...
  if ((configPage2.pinMapping == 255) ||
      (configPage2.pinMapping ==
       0))  // 255 = EEPROM value in a blank AVR; 0 = EEPROM value in new FRAM
  {
...
// Must come after setPinMapping() as secondary serial can be changed on a per
// board basis
  ...
  BIT_CLEAR(
      ...
      PROTECT_IO_ERROR);  // Clear the I/O error bit. The bit will be set in
                          // initialiseADC() if there is problem in there.
  initialiseADC();
  initialiseProgrammableIO();

  // Check whether the flex sensor is enabled and if so, attach an interrupt for
  // it
  ...
  if (configPage2.vssMode >
      1)  // VSS modes 2 and 3 are interrupt drive (Mode 1 is CAN)
  {
    attachInterrupt(digitalPinToInterrupt(pinVSS), vssPulse, RISING);
  }

  // Once the configs have been loaded, a number of one time calculations can be
  // completed
  req_fuel_uS =
      configPage2.reqFuel * 100;  // Convert to uS and an int. This is the only
                                  // variable to be used in calculations
  inj_opentime_uS =
      configPage2.injOpen *
      100;  // Injector open time. Comes through as ms*10 (Eg 15.5ms = 155).

  if (configPage10.stagingEnabled == true) {
    uint32_t totalInjector =
        configPage10.stagedInjSizePri + configPage10.stagedInjSizeSec;
    /*
        These values are a percentage of the req_fuel value that would be
       required for each injector channel to deliver that much fuel. Eg: Pri
       injectors are 250cc Sec injectors are 500cc Total injector capacity =
       750cc

        staged_req_fuel_mult_pri = 300% (The primary injectors would have to run
       3x the overall PW in order to be the equivalent of the full 750cc
       capacity staged_req_fuel_mult_sec = 150% (The secondary injectors would
       have to run 1.5x the overall PW in order to be the equivalent of the full
       750cc capacity
    */
    staged_req_fuel_mult_pri =
        (100 * totalInjector) / configPage10.stagedInjSizePri;
    staged_req_fuel_mult_sec =
        (100 * totalInjector) / configPage10.stagedInjSizeSec;
  }
  ...
  }  // set the secondary trigger edge automatically to correct working value
     // with poll level mode to enable cam angle detection in closed loop vvt.
  // Explanation: currently cam trigger for VVT is only captured when revolution
  // one == 1. So we need to make sure that the edge trigger happens on the
  // first revolution. So now when we set the poll level to be low on revolution
  // one and it's checked at tooth #1. This means that the cam signal needs to
  // go high during the first revolution to be high on next revolution at tooth
  // #1. So poll level low = cam trigger edge rising.

  // Begin the main crank trigger interrupt pin setup
  // The interrupt numbering is a bit odd - See here for reference:
  // arduino.cc/en/Reference/AttachInterrupt These assignments are based on the
  // Arduino Mega AND VARY BETWEEN BOARDS. Please confirm the board you are
  // using and update accordingly.
  currentStatus.RPM = 0;
  ...
  currentStatus.crankRPM =
      ((unsigned int)configPage4.crankRPM *
       10);  // Crank RPM limit (Saves us calculating this over and over again.
             // It's updated once per second in timers.ino)
  currentStatus.fuelPumpOn = false;
  currentStatus.engineProtectStatus = 0;
  triggerFilterTime =
      0;  // Trigger filter time is the shortest possible time (in uS) that
  ...
  currentStatus.nChannels =
      ((uint8_t)INJ_CHANNELS << 4) +
      IGN_CHANNELS;  // First 4 bits store the number of injection channels, 2nd
                     // 4 store the number of ignition channels
  fpPrimeTime = 0;
  ms_counter = 0;
  ...
  // The secondary input can be used for VSS if nothing else requires it. Allows
  // for the standard VR conditioner to be used for VSS. This MUST be run after
  // the initialiseTriggers() function
  if (VSS_USES_RPM2()) {
    attachInterrupt(digitalPinToInterrupt(pinVSS), vssPulse, RISING);
  ...
  if (configPage2.strokes == FOUR_STROKE) {
    // Default is 1 squirt per revolution, so we halve the given req-fuel figure
    // (Which would be over 2 revolutions)
    req_fuel_uS =
        req_fuel_uS /
        2;  // The req_fuel calculation above gives the total required fuel (At
            // VE 100%) in the full cycle. If we're doing more than 1 squirt per
            // cycle then we need to split the amount accordingly. (Note that in
            // a non-sequential 4-stroke setup you cannot have less than 2
            // squirts as you cannot determine the stroke to make the single
            // squirt on)
  }
  ...
  }  // Safety check. Should never happen as TS will give an error, but leave in
     // case tune is manually altered etc.
  ...
  switch (configPage2.nCylinders) {
    case 1:
      ...
      // Sequential ignition works identically on a 1 cylinder whether it's odd
      // or even fire.
    ...
    case 2:
      ...
      // Sequential ignition works identically on a 2 cylinder whether it's odd
      // or even fire (With the default being a 180 degree second cylinder).
      if ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) &&
          (configPage2.strokes == FOUR_STROKE)) {
        CRANK_ANGLE_MAX_IGN = 720;
      }
      ...
      // The below are true regardless of whether this is running sequential or
      // not
      if (configPage2.engineType == EVEN_FIRE) {
    ...
    case 3:
      ...
      maxInjOutputs = 3;
      if (configPage2.engineType == EVEN_FIRE) {
        // Sequential and Single channel modes both run over 720 crank degrees,
        // but only on 4 stroke engines.
        if (((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) ||
      ...
      // For alternating injection, the squirt occurs at different times for
      // each channel
      if ((configPage2.injLayout == INJ_SEMISEQUENTIAL) ||
        ...
        if (configPage2.injType == INJ_TYPE_PORT) {
          // Force nSquirts to 2 for individual port injection. This prevents
          // TunerStudio forcing the value to 3 even when this isn't wanted.
          currentStatus.nSquirts = 2;
          if (configPage2.strokes == FOUR_STROKE) {
            CRANK_ANGLE_MAX_INJ = 360;
          } else {
        ...
        if (currentStatus.nSquirts > 2) {
          channel2InjDegrees =
              (channel2InjDegrees * 2) / currentStatus.nSquirts;
          channel3InjDegrees =
              (channel3InjDegrees * 2) / currentStatus.nSquirts;
        }
    ...
    case 4:
      ...
      if (configPage2.engineType == EVEN_FIRE) {
        ...
        if (configPage4.sparkMode == IGN_MODE_ROTARY) {
          // Rotary uses the ign 3 and 4 schedules for the trailing spark. They
          // are offset from the ign 1 and 2 channels respectively and so use
          // the same degrees as them
          channel3IgnDegrees = 0;
          channel4IgnDegrees = 180;
          maxIgnOutputs = 4;

          configPage4.IgInv =
              GOING_LOW;  // Force Going Low ignition mode (Going high is never
                          // used for rotary)
        }
      } else {
        channel2IgnDegrees = configPage2.oddfire2;
...
#else
// This is an invalid config as there are not enough outputs to support
// sequential + staging Put the staging output to the non-existant channel 5
#if (INJ_CHANNELS >= 5)
          maxInjOutputs = 5;
          channel5InjDegrees = channel1InjDegrees;
#endif
...
#endif
      maxIgnOutputs = 5;  // Only 4 actual outputs, so that's all that can be
                          // cut
    ...
    case 6:
      ...
      if (configPage2.injLayout == INJ_SEQUENTIAL) {
        ...
        currentStatus.nSquirts = 1;
        req_fuel_uS = req_fuel_uS * 2;
      } else if (configPage10.stagingEnabled ==
                 true)  // Check if injector staging is enabled
      {
        maxInjOutputs = 6;
...
// Staging with 6 cylinders semi/sequential requires 7 total channels
          ...
          // support sequential + staging No staging output will be active
          maxInjOutputs = 6;
#endif
        }
      }
      ...
      if ((configPage2.injLayout == INJ_SEMISEQUENTIAL) ||
        ...
        if (!configPage2.injTiming) {
          ...
          channel3InjDegrees =
              (channel3InjDegrees * 2) / currentStatus.nSquirts;
          channel4InjDegrees =
              (channel4InjDegrees * 2) / currentStatus.nSquirts;
        }
      }
#if INJ_CHANNELS >= 8
      else if (configPage2.injLayout == INJ_SEQUENTIAL) {
  ...
  currentStatus.status3 |=
      ...
      << BIT_STATUS3_NSQUIRTS1;  // Top 3 bits of the status3 variable are the
                                 // number of squirts. This must be done after
                                 // the above section due to nSquirts being
                                 // forced to 1 for sequential

  // Special case:
  // 3 or 5 squirts per cycle MUST be tracked over 720 degrees. This is because
  // the angles for them (Eg 720/3=240) are not evenly divisible into 360 This
  // is ONLY the case on 4 stroke systems
  ...
  switch (configPage2.injLayout) {
    ...
    case INJ_SEMISEQUENTIAL:
      // Semi-Sequential injection. Currently possible with 4, 6 and 8
      // cylinders. 5 cylinder is a special case
      ...
      } else if (configPage2.nCylinders ==
                 5)  // This is similar to the paired injection but uses five
                     // injector outputs instead of four
      {
        fuelSchedule1.pStartFunction = openInjector1;
        fuelSchedule1.pEndFunction = closeInjector1;
        fuelSchedule2.pStartFunction = openInjector2;
  ...
  switch (configPage4.sparkMode) {
    ...
    case IGN_MODE_WASTEDCOP:
      // Wasted COP mode. Note, most of the boards can only run this for 4-cyl
      // only.
      if (configPage2.nCylinders <= 3) {
        ...
        // Wasted COP mode for 4 cylinders. Ignition channels 1&3 and 2&4 are
        // paired together
        ...
        // Wasted COP mode for 6 cylinders. Ignition channels 1&4, 2&5 and 3&6
        // are paired together
        ...
        // Wasted COP mode for 8 cylinders. Ignition channels 1&5, 2&6, 3&7 and
        // 4&8 are paired together
        ...
        // 4 cylinders or other than 6 cylinders, just use standard Wasted spark
        // mode
        ignitionSchedule1.pStartCallback = beginCoil1Charge;
        ignitionSchedule1.pEndCallback = endCoil1Charge;
        ignitionSchedule2.pStartCallback = beginCoil2Charge;
    ...
    case IGN_MODE_ROTARY:
      if (configPage10.rotaryType == ROTARY_IGN_FC) {
        // Ignition channel 1 is a wasted spark signal for leading signal on
        // both rotors
        ignitionSchedule1.pStartCallback = beginCoil1Charge;
        ignitionSchedule1.pEndCallback = endCoil1Charge;
        ignitionSchedule2.pStartCallback = beginCoil1Charge;
      ...
      }  // No action for other RX ignition modes (Future expansion / MISRA
         // compliant).
      break;
  ...
  // Begin priming the fuel pump. This is turned off in the low resolution, 1s
  // interrupt in timers.ino First check that the priming time is not 0
  ...
  interrupts();
  readCLT(false);  // Need to read coolant temp to make priming pulsewidth work
                   // correctly. The false here disables use of the filter
  ...
  /* SweepMax is stored as a byte, RPM/100. divide by 60 to convert min to sec
     (net 5/3).  Multiply by ignition pulses per rev. tachoSweepIncr is also the
     number of tach pulses per second */
 ...
 * The boardID is switch-case compared against raw boardID integers (not enum or
 * defined label, and probably no need for that either) which are originated
 * from tuning SW (e.g. TS) set values and are available in
 * reference/speeduino.ini (See pinLayout, note also that numbering is not
 * contiguous here).
 */
void setPinMapping(byte boardID) {
  ...
  switch (boardID) {
      // Note: Case 0 (Speeduino v0.1) was removed in Nov 2020 to handle default
      // case for blank FRAM modules
    ...
    case 3:
      ...
      // These are only to prevent lockups or weird behaviour on T4.1 when this
      // board is used as the default
      pinBaro = A4;
      pinMAP = A5;
...
// This is NOT correct. It has not yet been tested with this board
    ...
    case 9:
      ...
      // Note that there is no stepper driver output on the PNP boards. These
      // pins are unconnected and remain here just to prevent issues with random
      // pin numbers occurring
    ...
    case 10:
      ...
      pinSpareTemp1 = A6;
      pinSpareTemp2 = A5;
      pinTachOut = 41;   // Tacho output pin transistor is missing 2n2222 for
                         // this and 1k for 12v
    ...
    case 20:
#if defined(CORE_AVR) && \
    !defined(SMALL_FLASH_MODE)  // No support for bluepill here anyway
    ...
    case 30:
      ...
      pinTrigger = 19;       // The CAS pin
      pinTrigger2 = 18;      // The Cam Sensor pin
      pinTrigger3 = 17;      // cam sensor 2 pin, pin17 isn't external trigger
                             // enabled in arduino mega??
      pinTPS = A2;           // TPS input pin
      pinMAP = A3;           // MAP sensor pin
      pinIAT = A0;           // IAT sensor pin
      pinCLT = A1;           // CLS sensor pin
...
// Pin mappings for the BMW PnP PCBs by pazi88.
    ...
    case 40:
      ...
      pinFlex = 20;  // Flex sensor (Must be external interrupt enabled) - ONLY
                     // WITH DB
    ...
    case 41:
      ...
      pinInjector5 =
          45;           // Output pin injector 5 is on PLACEHOLDER value for now
    ...
    case 42:
      ...
      pinSpareLOut2 = 16;    // low current output spare2 PLACEHOLDER value for
                             // now
      pinSpareLOut3 = 17;    // low current output spare3 PLACEHOLDER value for
                             // now
      pinSpareLOut4 = 21;    // low current output spare4 PLACEHOLDER value for
                             // now
...
// Pin mappings for the DIY-EFI CORE4 Module. This is an AVR only module
    ...
    case 55:
      ...
      // The injector pins below are not used directly as the control is via SPI
      // through the MC33810s, however the pin numbers are set to be the SPI
      // pins (SCLK, MOSI, MISO and CS) so that nothing else will set them as
      // inputs
      ...
      */
      ...
      pinTrigger = 19;      // The CAS pin
      pinTrigger2 = 18;     // The Cam Sensor pin
      pinTrigger3 = 22;     // Uses one of the protected spare digitial inputs.
                            // This must be set or Serial1 (Pin 0) gets broken
      ...
      pSecondarySerial = &Serial1;  // Header that is broken out on Dropbear
                                    // boards is attached to Serial1
#endif
      ...
      pinResetControl =
          49;  // PLaceholder only. Cannot use 42-47 as these are the SD card
  ...
  if ((configPage6.launchPin != 0) &&
      (configPage6.launchPin < BOARD_MAX_IO_PINS)) {
    pinLaunch = pinTranslate(configPage6.launchPin);
  }
  if ((configPage4.ignBypassPin != 0) &&
      (configPage4.ignBypassPin < BOARD_MAX_IO_PINS)) {
    pinIgnBypass = pinTranslate(configPage4.ignBypassPin);
  }
  if ((configPage2.tachoPin != 0) &&
      (configPage2.tachoPin < BOARD_MAX_IO_PINS)) {
    pinTachOut = pinTranslate(configPage2.tachoPin);
  }
  if ((configPage4.fuelPumpPin != 0) &&
      (configPage4.fuelPumpPin < BOARD_MAX_IO_PINS)) {
  ...
  if ((configPage6.boostPin != 0) &&
      (configPage6.boostPin < BOARD_MAX_IO_PINS)) {
  ...
  if ((configPage6.useExtBaro != 0) &&
      (configPage6.baroPin < BOARD_MAX_IO_PINS)) {
    pinBaro = pinTranslateAnalog(configPage6.baroPin);
  }
  if ((configPage6.useEMAP != 0) &&
      (configPage10.EMAPPin < BOARD_MAX_IO_PINS)) {
    pinEMAP = pinTranslateAnalog(configPage10.EMAPPin);
  }
  if ((configPage10.fuel2InputPin != 0) &&
      (configPage10.fuel2InputPin < BOARD_MAX_IO_PINS)) {
    pinFuel2Input = pinTranslate(configPage10.fuel2InputPin);
  }
  if ((configPage10.spark2InputPin != 0) &&
      (configPage10.spark2InputPin < BOARD_MAX_IO_PINS)) {
  ...
  if ((configPage10.fuelPressureEnable) &&
      (configPage10.fuelPressurePin < BOARD_MAX_IO_PINS)) {
    pinFuelPressure = pinTranslateAnalog(configPage10.fuelPressurePin);
  }
  if ((configPage10.oilPressureEnable) &&
      (configPage10.oilPressurePin < BOARD_MAX_IO_PINS)) {
    pinOilPressure = pinTranslateAnalog(configPage10.oilPressurePin);
  }

  if ((configPage10.wmiEmptyPin != 0) &&
      (configPage10.wmiEmptyPin < BOARD_MAX_IO_PINS)) {
    pinWMIEmpty = pinTranslate(configPage10.wmiEmptyPin);
  }
  if ((configPage10.wmiIndicatorPin != 0) &&
      (configPage10.wmiIndicatorPin < BOARD_MAX_IO_PINS)) {
    pinWMIIndicator = pinTranslate(configPage10.wmiIndicatorPin);
  }
  if ((configPage10.wmiEnabledPin != 0) &&
      (configPage10.wmiEnabledPin < BOARD_MAX_IO_PINS)) {
    pinWMIEnabled = pinTranslate(configPage10.wmiEnabledPin);
  }
  if ((configPage10.vvt2Pin != 0) &&
      (configPage10.vvt2Pin < BOARD_MAX_IO_PINS)) {
  ...
  if ((configPage15.airConCompPin != 0) &&
      (configPage15.airConCompPin < BOARD_MAX_IO_PINS)) {
    pinAirConComp = pinTranslate(configPage15.airConCompPin);
  }
  if ((configPage15.airConFanPin != 0) &&
      (configPage15.airConFanPin < BOARD_MAX_IO_PINS)) {
    pinAirConFan = pinTranslate(configPage15.airConFanPin);
  }
  if ((configPage15.airConReqPin != 0) &&
      (configPage15.airConReqPin < BOARD_MAX_IO_PINS)) {
    pinAirConRequest = pinTranslate(configPage15.airConReqPin);
  }

  /* Reset control is a special case. If reset control is enabled, it needs its
     initial state set BEFORE its pinMode. If that doesn't happen and reset
     control is in "Serial Command" mode, the Arduino will end up in a reset
     loop because the control pin will go low as soon as the pinMode is set to
     OUTPUT. */
  if ((configPage4.resetControlConfig != 0) &&
      (configPage4.resetControlPin < BOARD_MAX_IO_PINS)) {
  ...
  if ((ignitionOutputControl == OUTPUT_CONTROL_MC33810) ||
    ...
    if ((LED_BUILTIN != SCK) && (LED_BUILTIN != MOSI) && (LED_BUILTIN != MISO))
      pinMode(LED_BUILTIN, OUTPUT);  // This is required on as the LED pin can
                                     // otherwise be reset to an input
  }

  // CS pin number is now set in a compile flag.
  //  #ifdef USE_SPI_EEPROM
  //    //We need to send the flash CS (SS) pin if we're using SPI flash. It
  //    cannot read from globals. EEPROM.begin(USE_SPI_EEPROM);
  //  #endif
...
// And for inputs
  ...
  // This can help prevent pin conflicts that users aren't aware of with unused
  // functions
  if ((configPage2.flexEnabled > 0) && (!pinIsOutput(pinFlex))) {
    pinMode(pinFlex, INPUT);  // Standard GM / Continental flex sensor requires
                              // pullup, but this should be onboard. The
                              // internal pullup will not work (Requires ~3.3k)!
  }
  if ((configPage2.vssMode > 1) &&
      (!pinIsOutput(pinVSS)))  // Pin mode 1 for VSS is CAN
  {
  ...
  if ((pinAirConRequest > 0) && ((configPage15.airConEnable) == 1) &&
    ...
    if ((configPage15.airConReqPol) == 1) {
      ...
      // Normal
      //  Pin pulled to Ground is ON. Floating (internally pulled up to +5V) is
      //  OFF.
      pinMode(pinAirConRequest, INPUT_PULLUP);
    }
  }

  if ((pinAirConFan > 0) && ((configPage15.airConEnable) == 1) &&
      ((configPage15.airConFanEnabled) == 1)) {
 ...
 * - Set Interrupt numbers @ref triggerInterrupt, @ref triggerInterrupt2 and
 * @ref triggerInterrupt3  by pin their numbers (based on board CORE_* define)
 * - Call decoder specific setup function triggerSetup_*() (by @ref
 * config4.TrigPattern, set to one of the DECODER_* defines) and do any
 * additional initialisations needed.
 *
 * @todo Explain why triggerSetup_*() alone cannot do all the setup, but there's
 * ~10+ lines worth of extra init for each of decoders.
 */
void initialiseTriggers(void) {
  ...
  secondaryTriggerEdge = 0;  // This is optional and may not be changed below,
                             // depending on the decoder in use
  tertiaryTriggerEdge = 0;  // This is even more optional and may not be changed
                            // below, depending on the decoder in use
  ...
  switch (configPage4.TrigPattern) {
    case DECODER_MISSING_TOOTH:
      ...
      }  // Attach the crank trigger wheel interrupt (Hall sensor drags to
         // ground when triggering)
      else {
      ...
      if (configPage10.TrigEdgeThrd == 0) {
        tertiaryTriggerEdge = RISING;
      } else {
        tertiaryTriggerEdge = FALLING;
      }

      attachInterrupt(triggerInterrupt, triggerHandler, primaryTriggerEdge);

      if (BIT_CHECK(decoderState, BIT_DECODER_HAS_SECONDARY)) {
        attachInterrupt(triggerInterrupt2, triggerSecondaryHandler,
                        secondaryTriggerEdge);
      }
      if (configPage10.vvt2Enabled > 0) {
        attachInterrupt(triggerInterrupt3, triggerTertiaryHandler,
                        tertiaryTriggerEdge);
    ...
    case DECODER_GM7X:
      ...
      if (configPage4.TrigEdge == 0) {
        attachInterrupt(triggerInterrupt, triggerHandler, RISING);
      }  // Attach the crank trigger wheel interrupt (Hall sensor drags to
         // ground when triggering)
      else {
        attachInterrupt(triggerInterrupt, triggerHandler, FALLING);
      }
    ...
    case DECODER_NON360:
      triggerSetup_non360();
      triggerHandler = triggerPri_DualWheel;  // Is identical to the dual wheel
                                              // decoder, so that is used. Same
                                              // goes for the secondary below
      triggerSecondaryHandler =
          triggerSec_DualWheel;  // Note the use of the Dual Wheel trigger
                                 // function here. No point in having the same
                                 // code in twice.
    ...
    case DECODER_36_2_2_2:
      ...
      getCrankAngle =
          getCrankAngle_missingTooth;  // This uses the same function as the
                                       // missing tooth decoder, so no need to
                                       // duplicate code
    ...
    case DECODER_VMAX:
      ...
      if (configPage4.TrigEdge == 0) {
        ...
        primaryTriggerEdge = false;
      }

      attachInterrupt(triggerInterrupt, triggerHandler,
                      CHANGE);  // Hardcoded change, the primaryTriggerEdge will
                                // be used in the decoder to select if it`s an
    ...
    case DECODER_SUZUKI_K6A:
      ...
      triggerHandler =
          triggerPri_SuzukiK6A;  // only primary, no secondary, trigger pattern
                                 // is over 720 degrees
...
static inline bool isAnyFuelScheduleRunning(void) {
  return fuelSchedule1.Status == RUNNING || fuelSchedule2.Status == RUNNING ||
         fuelSchedule3.Status == RUNNING || fuelSchedule4.Status == RUNNING
#if INJ_CHANNELS >= 5
...
void changeHalfToFullSync(void) {
  // Need to do another check for injLayout as this function can be called from
  // ignition
  noInterrupts();
  if ((configPage2.injLayout == INJ_SEQUENTIAL) &&
      (CRANK_ANGLE_MAX_INJ != 720) && (!isAnyFuelScheduleRunning())) {
    CRANK_ANGLE_MAX_INJ = 720;
    req_fuel_uS *= 2;
  ...
  // Need to do another check for sparkMode as this function can be called from
  // injection
  if ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) &&
      (CRANK_ANGLE_MAX_IGN != 720) && (!isAnyIgnScheduleRunning())) {
    CRANK_ANGLE_MAX_IGN = 720;
