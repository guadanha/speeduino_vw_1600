...
void calculateSecondaryFuel(void) {
  if (!configPage10.fuel2Mode) {
    return;
  }
  // If the secondary fuel table is in use, also get the VE value from there
  BIT_CLEAR(currentStatus.status3,
            BIT_STATUS3_FUEL2_ACTIVE);  // Clear the bit indicating that the 2nd
                                        // fuel table is in use.
  if (configPage10.fuel2Mode == FUEL2_MODE_MULTIPLY) {
    currentStatus.VE2 = getVE2();
    // Fuel 2 table is treated as a % value. Table 1 and 2 are multiplied
    // together and divided by 100
    uint16_t combinedVE =
        ((uint16_t)currentStatus.VE1 * (uint16_t)currentStatus.VE2) / 100;
    if (combinedVE <= 255) {
    ...
    currentStatus.VE2 = getVE2();
    // Fuel tables are added together, but a check is made to make sure this
    // won't overflow the 8-bit VE value
    uint16_t combinedVE =
        (uint16_t)currentStatus.VE1 + (uint16_t)currentStatus.VE2;
    if (combinedVE <= 255) {
      currentStatus.VE = combinedVE;
    } else {
      currentStatus.VE = 255;
    ...
    if (configPage10.fuel2SwitchVariable == FUEL2_CONDITION_RPM) {
      if (currentStatus.RPM > configPage10.fuel2SwitchValue) {
        BIT_SET(currentStatus.status3,
                BIT_STATUS3_FUEL2_ACTIVE);  // Set the bit indicating that the
                                            // 2nd fuel table is in use.
        currentStatus.VE2 = getVE2();
    ...
    if (digitalRead(pinFuel2Input) == configPage10.fuel2InputPolarity) {
      BIT_SET(currentStatus.status3,
              BIT_STATUS3_FUEL2_ACTIVE);  // Set the bit indicating that the 2nd
                                          // fuel table is in use.
      currentStatus.VE2 = getVE2();
      currentStatus.VE = currentStatus.VE2;
    }
...
void calculateSecondarySpark(void) {
  if (!configPage10.spark2Mode) {
    return;
  }

  // Same as above but for the secondary ignition table
  BIT_CLEAR(currentStatus.spark2,
            BIT_SPARK2_SPARK2_ACTIVE);  // Clear the bit indicating that the 2nd
                                        // spark table is in use.
  ...
  if (configPage10.spark2Mode == SPARK2_MODE_MULTIPLY) {
    ...
    // make sure we don't have a negative value in the multiplier table (sharing
    // a signed 8 bit table)
    if (currentStatus.advance2 < 0) {
      currentStatus.advance2 = 0;
    }
    // Spark 2 table is treated as a % value. Table 1 and 2 are multiplied
    // together and divided by 100
    int16_t combinedAdvance =
        ((int16_t)currentStatus.advance1 * (int16_t)currentStatus.advance2) /
        100;
    ...
    BIT_SET(currentStatus.spark2,
            BIT_SPARK2_SPARK2_ACTIVE);  // Set the bit indicating that the 2nd
                                        // spark table is in use.
    currentStatus.advance2 = getAdvance2();
    ...
    if (configPage10.spark2SwitchVariable == SPARK2_CONDITION_RPM) {
      if (currentStatus.RPM > configPage10.spark2SwitchValue) {
        BIT_SET(currentStatus.spark2,
                BIT_SPARK2_SPARK2_ACTIVE);  // Set the bit indicating that the
                                            // 2nd spark table is in use.
        currentStatus.advance2 = getAdvance2();
  ...
  // Apply the fixed timing correction manually. This has to be done again here
  // if any of the above conditions are met to prevent any of the seconadary
  // calculations applying instead of fixec timing
  currentStatus.advance = correctionFixedTiming(currentStatus.advance);
  currentStatus.advance = correctionCrankingFixedTiming(
      currentStatus
          .advance);  // This overrides the regular fixed timing, must come last
}
